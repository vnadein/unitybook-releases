...
-   **Базовый URL:** Все эндпоинты API начинаются с `/api/external`.
...
### Получение и использование JWT-токена
...
`/api/external/{ТипОбъекта}/{ИмяОбъекта}`
`/api/external/{ТипОбъекта}/{ИмяОбъекта}/{IDЗаписи}`
...
`GET /api/external/{ТипОбъекта}/{ИмяОбъекта}`
...
`GET /api/external/Справочник/Контрагенты?ИНН[ne]=&_sort=Наименование:asc&_page=2&_limit=20`

### 3.2. Чтение одной записи (`GET`)
`GET /api/external/{ТипОбъекта}/{ИмяОбъекта}/{IDЗаписи}`

### 3.3. Создание записи (`POST`)
`POST /api/external/{ТипОбъекта}/{ИмяОбъекта}`
...
`DELETE /api/external/{ТипОбъекта}/{ИмяОбъекта}/{IDЗаписи}`
...
`POST /api/external/Документ/{ИмяОбъекта}/{id}/post`
...
`POST /api/external/Документ/{ИмяОбъекта}/{id}/unpost`
